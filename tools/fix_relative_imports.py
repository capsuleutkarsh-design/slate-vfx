
import os
import re
from pathlib import Path

ROOT = Path("ut_vfx/core")
DOMAIN_DIR = ROOT / "domain"
INFRA_DIR = ROOT / "infra"

# Map: Module -> Package
MODULE_LOCATIONS = {}
for f in os.listdir(INFRA_DIR):
    if f.endswith(".py"): MODULE_LOCATIONS[f[:-3]] = "infra"
for f in os.listdir(DOMAIN_DIR):
    if f.endswith(".py"): MODULE_LOCATIONS[f[:-3]] = "domain"

def fix_file(filepath, current_pkg):
    try:
        content = filepath.read_text(encoding="utf-8")
        original_content = content
        
        # Regex for 'from .MODULE import ...'
        # Group 1: MODULE
        # Group 2: remainder
        
        lines = content.splitlines()
        new_lines = []
        
        for line in lines:
            # Handle: from .global_config import GlobalConfig
            match = re.search(r'^from \.(\w+) import (.*)', line)
            if match:
                mod_name = match.group(1)
                imports = match.group(2)
                
                target_pkg = MODULE_LOCATIONS.get(mod_name)
                
                if target_pkg and target_pkg != current_pkg:
                    print(f"Fixing relative import in {filepath.name}: {mod_name} is in {target_pkg}")
                    # Change 'from .mod' to 'from ..target.mod'
                    new_line = f"from ..{target_pkg}.{mod_name} import {imports}"
                    new_lines.append(new_line)
                    continue
            
            new_lines.append(line)
        
        new_content = "\n".join(new_lines)
        if new_content != original_content:
            filepath.write_text(new_content, encoding="utf-8")
            
    except Exception as e:
        print(f"Error checking {filepath}: {e}")

print("--- Fixing Relative Imports ---")

if DOMAIN_DIR.exists():
    for f in os.listdir(DOMAIN_DIR):
        if f.endswith(".py"): fix_file(DOMAIN_DIR / f, "domain")

if INFRA_DIR.exists():
    for f in os.listdir(INFRA_DIR):
        if f.endswith(".py"): fix_file(INFRA_DIR / f, "infra")

print("--- Done ---")
