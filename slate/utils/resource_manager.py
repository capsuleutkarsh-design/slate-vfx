"""
Resource management utilities for Slate Production tool.
...
class ResourcePathManager:
    ...
    def get_resource_path(relative_path: str) -> Path:
        ...
        try:
            ...
            if hasattr(sys, '_MEIPASS'):
                ...
                # Check 1: Nested in 'slate' (e.g. icons) under internal
                p1 = base_path / "slate" / relative_path
            ...
            if getattr(sys, 'frozen', False):
                ...
                # Your batch file puts resources in 'slate/icons', check that first
                if (base_path / "slate" / relative_path).exists():
                    return base_path / "slate" / relative_path
                return base_path / relative_path

            # 3. Development mode
            # Assumes this file is in slate/utils/
    ...
    def _local_tool_dir() -> Path:
        if os.name == "nt":
            ...
            if root:
                return Path(root) / "Slate" / "bin"
            return Path.home() / "AppData" / "Local" / "Slate" / "bin"
        return Path.home() / ".slate" / "bin"

    @staticmethod
    def _tool_candidates(tool_name: str):
        tool_bin = ResourcePathManager._tool_binary_name(tool_name)
        env_key = f"Slate_{tool_name.upper()}_PATH"
