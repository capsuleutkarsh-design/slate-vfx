"""
SECURE Configuration manager for Slate Production tool.
...
from slate.utils.security import SecurityValidator, SecurityError
...
class ConfigManager:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            if not dir_valid:
                ...
                self.app_data_dir = Path.home() / ".slate_secure"
                self.app_data_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            logging.exception(f"App data directory validation failed: {e}")
            self.app_data_dir = Path.home() / ".slate_secure_fallback"
            self.app_data_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _get_app_data_dir(self) -> Path:
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                app_dir = Path(os.environ.get('LOCALAPPDATA', Path.home() / "AppData" / "Local")) / "Slate"
            else:
                # Running as script
                app_dir = Path(os.environ.get('LOCALAPPDATA', Path.home() / "AppData" / "Local")) / "Slate"
            ...
            fallback_dir = Path.home() / ".slate_secure_fallback"
    ...
    def _load_default_templates(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Assuming your package is named 'slate' and the file is in 'slate.data'
            template_path = files('slate.data').joinpath('templates.json')
