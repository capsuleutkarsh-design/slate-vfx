"""
OpenAssetIO Backend — Adapter for Slate Asset Management

Wraps the existing LibraryManager behind the OpenAssetIO ManagerInterface,
enabling Slate to participate in standardised VFX asset resolution workflows.
...
class OpenAssetIOBackend:
    """
    Bridges the OpenAssetIO ManagerInterface to the Slate LibraryManager.
        ...
        if self._oaio is not None:
            try:
                ...
                if hasattr(self._oaio, 'initialize') and not self._initialized:
                    # Pass minimal settings — Slate doesn't use external OAIO hosts yet
    ...
    def resolve(self, entity_reference: str) -> Optional[Dict[str, Any]]:
        ...
        In Slate, an entity reference is the asset's file_path.
    ...
    def is_entity_reference(self, token: str) -> bool:
        ...
        if self._oaio and hasattr(self._oaio, 'isEntityReferenceString'):
            try:
                ...
                pass
        # Heuristic: any absolute path or Slate-style reference
...
def try_create_openassetio_backend(library_manager) -> Optional[OpenAssetIOBackend]:
    ...
    try:
        ...
        # Create a minimal host interface for Slate
        class SlateHostInterface(HostInterface):
            def identifier(self):
                return "org.utcap.host"

            def displayName(self):
                return "Slate VFX Pipeline"

        host_interface = SlateHostInterface()
        ...
        try:
            ...
            if available:
                ...
                logger.info("OpenAssetIO: No external managers found, using Slate adapter only")
                return OpenAssetIOBackend(library_manager, manager_interface=None)

        except Exception as e:
            logger.info(f"OpenAssetIO: Manager creation skipped ({e}), using Slate adapter")
            return OpenAssetIOBackend(library_manager, manager_interface=None)
