...
class SidecarEngine(QObject):
    ...
    def __init__(self, manifest):
        ...
        self.download_dir = Path(os.getenv('TEMP')) / "SlateUpdate"
    ...
    def stage_update(self, source_zip_path=None):
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                updater_exe = base_dir / "SlateUpdater.exe"
            else:
                base_dir = Path.cwd()
                updater_exe = base_dir / "SlateUpdater.exe"
            ...
            if not updater_exe.exists():
                ...
                logging.warning("SlateUpdater.exe not found! This is expected in dev but fatal in production.")
                # We will continue staging but apply_update will fail.
                
            # 3. Stage Updater in Temp
            self.temp_updater = self.download_dir / "SlateUpdater.exe"
    ...
    def apply_update(self):
        ...
        try:
            ...
            if not self.temp_updater.exists():
                raise FileNotFoundError("Staged SlateUpdater.exe not found.")
