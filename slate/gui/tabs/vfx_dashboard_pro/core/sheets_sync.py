...
class GoogleSheetsSync:
    ...
    def connect(self, sheet_url: str) -> bool:
        ...
        if not creds_path:
            self.last_error = (
                ...
                "Set Slate_GOOGLE_CREDENTIALS or configure "
    ...
    def _credential_candidates(self) -> Iterable[Path]:
        configured = [
            ...
            os.getenv("Slate_GOOGLE_CREDENTIALS", ""),
        ...
        if localapp:
            fallback_paths.append(Path(localapp) / "Slate" / "credentials" / "google_service_account.json")
            fallback_paths.append(Path(localapp) / "Slate" / "credentials" / "credentials.json")
