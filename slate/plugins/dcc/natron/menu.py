...
def load_plate(hotkey_type):
    ...
    data = get_shot_data()
    if not data:
        app.message("Slate Error: Shot context not found in environment.")
        return
        
    scan_path = data.get("scan_path", "")
    if not scan_path:
        app.message("Slate Error: Scan path not defined in database.")
        return
    ...
    if not shot_root:
        app.message("Slate Error: Could not determine Shot Root directory.")
        return
    ...
    if seq_path:
        ...
        app.message(f"Slate: No media found in {target_folder}")
...
def save_script_version(department):
    ...
    data = get_shot_data()
    if not data:
        app.message("Slate Error: Shot context not found.")
        return
    ...
    try:
        ...
        app.message(f"Slate: Successfully saved new version:\n{filename}")
    except Exception as e:
