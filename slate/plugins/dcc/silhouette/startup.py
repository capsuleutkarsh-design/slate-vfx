...
class SlateLoadPlateAction(fx.Action):
    def __init__(self):
        super().__init__("Slate|Load Scan Plate")

    def execute(self):
        data = get_shot_data()
        if not data:
            fx.displayError("Slate Error", "Shot context not found in environment.")
            return

        scan_path = data.get("scan_path", "")
        if not scan_path:
            fx.displayError("Slate Error", "Scan path not defined in database.")
            return
        ...
        if not shot_root:
            fx.displayError("Slate Error", "Could not determine Shot Root directory.")
            return
        ...
        if seq_path:
            try:
                ...
                if source:
                    fx.displayMessage("Slate", f"Loaded plate from: {seq_path}")
            except Exception as e:
                fx.displayError("Slate Load Error", str(e))
        else:
            fx.displayError("Slate", f"No media found in {target_folder}")

class SlateSaveVersionAction(fx.Action):
    def __init__(self):
        super().__init__("Slate|Save Roto Version")

    def execute(self):
        data = get_shot_data()
        if not data:
            fx.displayError("Slate Error", "Shot context not found.")
            return
        ...
        try:
            ...
            if fx.app.session():
                ...
                fx.displayMessage("Slate", f"Successfully saved new version:\n{filename}")
            else:
                fx.displayError("Slate", "No active session to save.")
        except Exception as e:
            fx.displayError("Slate Save Error", str(e))

# Register actions when Silhouette starts
try:
    fx.addAction(SlateLoadPlateAction())
    fx.addAction(SlateSaveVersionAction())
    print("Slate Silhouette Plugin Initialized.")
except NameError:
    pass
