...
def get_shot_data():
    ...
    try:
        ...
        nuke.tprint(f"Slate DB Error: {e}")
...
def load_plate(hotkey_type):
    ...
    data = get_shot_data()
    if not data:
        nuke.message("Slate Error: Shot context not found in environment.")
        return
    ...
    if not scan_path:
        nuke.message("Slate Error: Scan path not defined in database.")
        return
    ...
    if not shot_root:
        nuke.message("Slate Error: Could not determine Shot Root directory.")
        return
    ...
    if seq_path:
        ...
        nuke.message(f"Slate: No media found in {target_folder}")
...
def save_script_version(department):
    ...
    data = get_shot_data()
    if not data:
        nuke.message("Slate Error: Shot context not found.")
        return
    ...
    try:
        ...
        nuke.message(f"Slate: Successfully saved new version:\n{filename}")
    except Exception as e:
        nuke.message(f"Slate Error saving script: {e}")
