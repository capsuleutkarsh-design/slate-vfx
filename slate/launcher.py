...
def main():
    ...
    # 2. Target Executable
    # In the installed structure, Slate.exe is in the same folder
    target_exe = application_path / "Slate.exe"
    ...
    if target_exe.exists():
        ...
        if getattr(sys, 'frozen', False):
            error_exit(f"Could not find Slate.exe at: {target_exe}")
        else:
            logging.info(f"Usage: This launcher expects Slate.exe at {target_exe}")
